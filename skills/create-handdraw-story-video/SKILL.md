...
6. Have the web model generate the style reference, character sheet, then all scenes autonomously in the same conversation. The master instruction must prohibit confirmation requests and extra uploads; if image reuse is unavailable, it must continue from the fixed visual constraints. Tell the user the exact keyframe folder displayed in the workspace; they must download every output there using the shown filenames.
